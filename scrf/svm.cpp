...
#ifdef MULTI
	int abortCommand, abort;
	MPI_Request request;
	MPI_Irecv(&abortCommand, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &request);
#endif
...
#ifdef MULTI
	  MPI_Test(&request, &abort, MPI_STATUS_IGNORE);
	  if (abort)  //master node told us to stop our computation 
	    break;
#endif
...
{
  //fprintf(stderr, "sigmoid_predict called with %lf, %lf, %lf\n", decision_value, A, B);
